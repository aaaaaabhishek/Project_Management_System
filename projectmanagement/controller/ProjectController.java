...
public class ProjectController {
    ...
    public ResponseEntity<Page<ProjectDto>> getAllProject(@RequestParam(defaultValue = "0") int page,@RequestParam(defaultValue = "20") int limit){
        ...
        if(projectDtos!=null){
            ...
            logger.info("Number of projects returned: {}", projectDtos.getSize());
    ...
    public ResponseEntity<ProjectDto> getProjectById(@PathVariable String projectId){
        ...
        if(projectDto!=null){
            logger.info("Project list  of the the data is extracted:{}",projectDto);
            return new ResponseEntity<>(projectDto,HttpStatus.ACCEPTED);
        }
        logger.warn("Project List extraction is failed");

        return new ResponseEntity<>(projectDto,HttpStatus.BAD_GATEWAY);
    }
