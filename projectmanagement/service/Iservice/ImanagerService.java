...
public class ImanagerService implements ManagerService {
    ...
    public ManagerDto addManager(ManagerDto managerDto) {
        if(managerRepository.findById(managerDto.getManagerId()).isPresent()) {
            throw new RuntimeException("Manager is already present with this manager_id: " + managerDto.getManagerId());
        }
//                .orElseThrow(()->new RuntimeException("Manager is already present with this manager_id: "+managerDto.getManagerId()));
